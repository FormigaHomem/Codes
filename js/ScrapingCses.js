...
function build(Codes, path){
    ...
    for(let [Category, Arr] of Codes.entries() ){
        ...
        for(let Code of Arr){
            ...
            fs.writeFile(dir+`\\${Category}\\${Code.problemName.replace(" ", "_")+".cpp"}`, Code.code, (err) => {
                if(err){
...
async function main(path, CategoryOf, Codes){
    ...
    await inquirer.prompt( [ 
        {   name: "option1", 
            ...
            choices: ["To create your Cses problems directory?", "To delete your Cses problems directory?"],
